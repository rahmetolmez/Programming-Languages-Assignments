...
(defun test (string0 stream)
	(let (lex0 stat kwFlag quotCounter val idname temp)
		(setf lex0 "")
		(setf stat "start")
		(setf quotCounter 0)
		...
		(dotimes (iter (length string0))
				(progn
					...
					(cond
						...
						((and (string= stat "identifier_pos") (alphanumericp (char string0 iter))) (progn (setf stat "identifier_pos")))
						...
						((string= stat "zero_pos") (progn (setf stat "zero")))
						((and (eql (char string0 iter) #\0 ) (not (string= stat "value_pos"))) (progn (setf stat "zero_pos") (setf val (concatenate 'string val (string (char string0 iter))))))
						...
						((and (string= stat "value_pos") (digit-char-p (char string0 iter))) (progn (setf stat "value_pos") (setf val (concatenate 'string val (string (char string0 iter))))))
						((digit-char-p (char string0 iter)) (progn (setf stat "value_pos") (setf val (concatenate 'string val (string (char string0 iter))))))
					...
					(if (string= stat "zero")
						(progn (format stream "VALUE ~d~%" val) (setf stat "start") (setf val "")))
					...
					(if (or (string= stat "start") (string= stat "identifier_pos"))
						(progn (setf lex0 (concatenate 'string lex0 (string (char string0 iter)))) (setf idname (concatenate 'string idname (string (char string0 iter)))))) ;add next char to word		
					...
					(if (or (string= stat "end") (string= stat "identifier"))
						(progn (setf kwFlag (tokenize-test lex0 stream)) (tokenize-value lex0 stream) (setf lex0 "") (setf idname "")))

					;function if above if did not print anything
					(if (and (not kwFlag) (string= stat "identifier"))
						(progn (format stream "IDENTIFIER ~A~%" temp) (setf stat "start") (setf idname "") (setf temp "")))
